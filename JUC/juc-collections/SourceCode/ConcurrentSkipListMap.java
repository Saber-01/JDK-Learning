...
public class ConcurrentSkipListMap<K,V> extends AbstractMap<K,V>
     ...
     * 初始化。
     */
    private void initialize() {  //初始化类中的变量为null，主要是使用BASE_HEADER作为head的底层node。level初始为1.
        keySet = null;
        entrySet = null;
        values = null;
        descendingMap = null;
    ...
    static class Index<K,V> {
         ...
         * @return true if successful
         */
        final boolean unlink(Index<K,V> succ) {    //底层node已经为null的情况下，跳过该succ,即删除对应的index。
    ...
    static final class HeadIndex<K,V> extends Index<K,V> {
        final int level;  //相对于普通索引节点，多了level属性，表示当前是第几层的HeadIndex.
    ...
    static final int cpr(Comparator c, Object x, Object y) {   //比较大小。
    ...
    /**
     * 返回节点的key根据比较器应该排在指定key前面的第一个有层数的节点的key，如果没有返回底层的头结点。
     * @param key the key
    ...
    private Node<K,V> findPredecessor(Object key, Comparator<? super K> cmp) {
        ...
        for (;;) {
            // 从索引的最高层的index节点 head开始遍历。初始q为head，r为head.right
            for (Index<K,V> q = head, r = q.right, d;;) {
                // 当前遍历的q节点的right不为 null
                if (r != null) {
                    Node<K,V> n = r.node;   //得到index节点r对应的底层的普通节点node。
                    K k = n.key;  //取出node的key
                    // 如果 r.Node 已经被删除，那么要调用unlink进行删除无效索引。
                    if (n.value == null) {
                        // 尝试 CAS 将q的右节点设置为 r.right（即跳过r到r.right,从而实现删除 r）
                        if (!q.unlink(r))
                            // 更新失败跳出循环，
                            break;
                        // 更新成功后，r重新赋值为 q 的当前右节点(上一个r的right)
                        r = q.right;
                        // 重新开始循环，不进入下一个if.
                        continue;
                    }
                    //直到找到一个index的node是有效的，才到这里。
                    // 比较 key 和 k，如果 key 大于 k(传入的比较器为升序的话),则还需继续循环
                    if (cpr(cmp, key, k) > 0) {
                        q = r; //此时q赋值为r。
                        r = r.right;  //r赋值为r.right。继续循环
                        continue;
                    }
                }
                //到这里，当前层向右找到了比当前key大的了，或者已经遍历到当前层的末尾，r=null了。那么需要向下换一层
                // 右节点为 null，或者 key 小于右节点的 key，向下查找
                // 下节点为 null，返回当前 q 节点的 node
                if ((d = q.down) == null)    //直到index的level是第一层，那么就返回这个index对应node。
                    return q.node;
                // 下节点不为 null，继续往下换一层，然后继续往右，直到右边index大于或遇到null。
                q = d;
                r = d.right;
            }
        }
    ...
    private Node<K,V> findNode(Object key) {
        ...
        outer: for (;;) {
            // b为指定 key 的前面有层数的第一个节点。然后向右遍历。
            for (Node<K,V> b = findPredecessor(key, cmp), n = b.next;;) {
                Object v; int c;
                // 到头了，直接打断内圈循环，没找到。
                if (n == null)
                    break outer;
                Node<K,V> f = n.next;  //保存这个节点之后的一个节点。
                // 有其他线程更改了b的节点，重新循环
                if (n != b.next)
                    break;
                // 已经被标记删除，调用 helpDelete 删除这个节点。然后调用findPredecessor重新循环。
                if ((v = n.value) == null) {
                    n.helpDelete(b, f);
                    break;
                }
                // b 已经被删除，findPredecessor重新循环
                if (b.value == null || v == n)
                    break;
                // 找到了
                if ((c = cpr(cmp, key, n.key)) == 0)
                    return n;
                if (c < 0)   //如果已经遍历到了的n.key。大于指定key。那么说明找不到了。break outer。退出自旋。
                    break outer;
                b = n; //更新b向后遍历
                n = f;  //更新n。
            }
    ...
    private V doGet(Object key) {
        ...
        return null;  //上面循环没有返回说明没找到。返回null。
    }
    ...
    private V doPut(K key, V value, boolean onlyIfAbsent) {
        ...
        // 第一部分：在最底层链表中插入节点
        outer: for (;;) {
            // findPredecessor 函数找到给定 key 的前继节点 b(有level的节点)
            for (Node<K,V> b = findPredecessor(key, cmp), n = b.next;;) {
                ...
                // 上面 if 所有条件都不满足（如果满足，要么 continue 要么 break，不会到这里）说明找到合适位置可以插入；
                // 或者直接 n == null，后面已经没有节点了，也可以插入。
                z = new Node<K,V>(key, value, n);   //使用指定key和value。并且遍历到的刚好大于key的节点或者null作为新建节点的next然后创建新节点
                // 把新节点添加 b 节点后面。
                if (!b.casNext(n, z))
                    // 失败则重新开始从findPredecessor方法开始遍历。
                    break;
                // 到这里说明添加成功。退出自旋。
                break outer;
            }
        ...
        if ((rnd & 0x80000001) == 0) {
            ...
            // 计算出来的层级小于等于最高层的层级，不需要增加HeadIndex，
            //如果level没有超过h.level。
            if (level <= (max = h.level)) {
                //在新建的节点上，建立level层索引，从下到上，从第一层到第level层。
                for (int i = 1; i <= level; ++i)
                    // z 是刚刚新添加的节点，idx 是 down（注意所有的索引节点都有
                    // node 属性，保存它对应的数据节点）
                    idx = new Index<K,V>(z, idx, null);
            }
            // 如果level比h.level还要大，说明HeadIndex需要增加。
            else {
                // 在最高层数基础上增加一层
                level = max + 1;
                ...
                // 从下到上构建值为 z 的索引节点，并且将索引节点存在数组对应的i里面
                for (int i = 1; i <= level; ++i)
                ...
                for (;;) {
                    ...
                    // 创建新的 head，将 down 指向原来的 head，将 right 指向插入的node上的新建的索引列。
                    // 此循环只会执行一次
                    ...
                    if (casHead(h, newh)) {
                        h = newh;
                        idx = idxs[level = oldLevel]; //将idx保存数组的oldLevel层的index节点。方便下面第三部分。
                        break;  //成功设置后打断自旋
                    }
                }
            }
            ...
            splice: for (int insertionLevel = level;;) {
                ...
                for (Index<K,V> q = h, r = q.right, t = idx;;) {
                    ...
                    if (r != null) {
                        ...
                        // 节点的 value 为 null，说明索引失效。调用unlink删除当前层的失效索引。
                        if (n.value == null) {
                            // 删除，如果删除失败，说明线程修改了map，则从head开始重新循环。
                            if (!q.unlink(r))
                                break;
                            // 删除成功后还是取右节点，然后结束本次循环。
                            r = q.right;
                            continue;
                        }
                        // 找到的key比插入的key要小还要往右边查找
                        if (c > 0) {
                            q = r;
                            r = r.right;
                            continue;
                    ...
                    if (j == insertionLevel) {
                        // 在 q 和 r 之间插入 t
                        if (!q.link(r, t))
                            break; // 如果失败了，说明有线程修改，则退出内层循环重新找。
                        // 如果插入完成后，t 索引节点被删除，那么结束插入操作，此时纵向索引的其他索引也不需要连接了。
                        if (t.node.value == null) {
                            findNode(key);
                            break splice;
                        }
                        // 更新insertionLevel，判断如果到达底层，那么插入完毕。
                        if (--insertionLevel == 0)
                            break splice;
                    }
                    ...
                    // 当前层级已经到最右边了，继续往下一层级
                    // q 向下移动一位
                    q = q.down;   //注意。q是已经向右移动过的。
                    r = q.right;
                }
            }
        }
        return null; //如果不是值覆盖，就返回null。
    }
    ...
    final V doRemove(Object key, Object value) {
        ...
        outer: for (;;) {
            ...
            for (Node<K,V> b = findPredecessor(key, cmp), n = b.next;;) {
                Object v; int c;
                // 已经遍历到尾部了，直接退出说明没有该节点。
                if (n == null)
                    break outer;
                // f 当前节点的后继节点，因为在遍历过程如果遇到失效的节点，将会调用helpDelete进行删除。
                Node<K,V> f = n.next;
                // 再次检查 n 不是 b.next，说明被其他线程修改过，重新从当前b和n开始。
                if (n != b.next)                    // inconsistent read
                    break;
                // n 被标记为删除状态
                if ((v = n.value) == null) {        // n is deleted
                ...
                // 进行到这里说明 c == 0，找到了要删除的节点 n，此时b为n前面的节点，f为n后面的节点
                // value 不等于 v，说明其他线程把 value 修改了，那么退出自旋。即不发生改变。
                if (value != null && !value.equals(v))
                    break outer;
                // 完成所有的检查，标记节点 n 为待删除节点
                // 使用CAS 将 n 的 value 设置为 null，如果失败，重新开始循环。
                if (!n.casValue(v, null))
                    break;
                // 尝试在 n 节点和f节点之间，添加标记节点（失败直接进入 if）
                ...
                if (!n.appendMarker(f) || !b.casNext(n, f))
                    ...
                    // 如果删除索引节点之后，最高层没有 right 了，需要使用tryReduceLevel维护Head的层数
                    if (head.right == null)
                        tryReduceLevel();
                }
                // 返回删除的元素值
    ...
    private void tryReduceLevel() {
        ...
        // 最上面的三层headIndex的 right 都等于 null 时才会尝试修改 head（最上面三层都空了的
        // 时候，才会降级）
        // 如果修改后再次检查，如果 h 的 right 又不为 null 了，就CAS尝试还原
        if (h.level > 3 &&
     ...
     * 构造函数
     */
    public ConcurrentSkipListMap() { //空构造函数
     ...
     */
    public ConcurrentSkipListMap(Comparator<? super K> comparator) {  //指定比较器的构造函数。
     ...
     *         or values are null
     */
    public ConcurrentSkipListMap(Map<? extends K, ? extends V> m) {  //使用指定map构造。
        this.comparator = null;
        initialize();  //初始化后
        putAll(m);   //调用putAll
    }
     ...
     */
    public ConcurrentSkipListMap(SortedMap<K, ? extends V> m) {  //使用排序的Map
    ...
    private void buildFromSorted(SortedMap<K, ? extends V> map) {
        ...
        //在每个级别跟踪当前最右侧的节点。使用ArrayList避免提交到初始或最大级别。
        //该数组中保存了每个level(存在索引i=level位置上)存放的最右侧的index，方便维护每一个level的连接。
        ArrayList<Index<K,V>> preds = new ArrayList<Index<K,V>>();

        // initialize
        for (int i = 0; i <= h.level; ++i)  //初始化ArrayList数组。
            preds.add(null);
        Index<K,V> q = h;
        for (int i = h.level; i > 0; --i) {   //从h.level开始，到1结束，i不能为0.
            preds.set(i, q);    //在每一层放入head。head是初始化过的HeadIndex
            q = q.down;
        }

        Iterator<? extends Map.Entry<? extends K, ? extends V>> it =
                map.entrySet().iterator();       //得到传入的Sortedmap的迭代器。
        while (it.hasNext()) {
            Map.Entry<? extends K, ? extends V> e = it.next();
            int rnd = ThreadLocalRandom.current().nextInt();
            int j = 0;
            //level=1条件。为。rnd=0.....001001010....00。中间1的个数随便。
            //level=2条件为，rnd=0000000...0000010,
            //level=3 条件为： rnd= 000000....0000110,以此类推。
            if ((rnd & 0x80000001) == 0) {  //排除随机数中的负数和奇数。
                do {
                    ++j;
                } while (((rnd >>>= 1) & 1) != 0);//向右移动后最后一位为1.就继续，
                if (j > h.level) j = h.level + 1;  //如果随机到的level大于了head的层数，那么j就为h.level+1。
            }
            K k = e.getKey();
            V v = e.getValue();
            if (k == null || v == null)   //因为并发性集合map不能存放key或value为null的映射，所以要判断
                throw new NullPointerException();
            Node<K,V> z = new Node<K,V>(k, v, null);  //创建一个普通节点。
            basepred.next = z;   //  如果是第一次加入，那么BASE_HEADER.next赋值为创建的普通节点，以后的遍历就是上一个普通节点。
            basepred = z;   //然后 更新为新建的普通节点，方便下次链表的连接
           //以下为维护index结构。
            if (j > 0) {//如果上面得到的j是大于0，说明需要创建索引层。
                Index<K,V> idx = null;  //创建一个索引空节点。
                for (int i = 1; i <= j; ++i) {   //从第一层开始，到j层
                    idx = new Index<K,V>(z, idx, null);  //新建索引节点，node为新建的node，down为上一个创建的index，right为null。
                    if (i > h.level)  //如果i比h.level还要大，说明，HeadIndex需要更新，使其多一层。
                        h = new HeadIndex<K,V>(h.node, h, idx, i); //使用原始h.node作为node.原始的h左右为down。right为新建的idex。

                    //这一步完成对前一步保存在ArrayList数组preds中的每一个层的最右侧索引点进行更新。
                    if (i < preds.size()) {   //索引还是在preds赋值过的范围，即存储了当前索引应该放
                        preds.get(i).right = idx;   //注意第i层新建的index也会连接在第i层的前一个index的后面。
                        preds.set(i, idx);     //连接了以后，存放第i层的最右侧的index将要更新。
                    } else
                        preds.add(idx);      //如果j大于h.level,上一步创建了新的HeadIndex的right赋值为了idx.所以新的一层的最右侧应该赋值为idx。
                }
            }
        }
        head = h;  //更新head。
